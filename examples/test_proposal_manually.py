...
workchain_node =  load_node(252989) # NiO 40 points
...
#%% Linear bandit test (ARD)

import lordcapulet.functions.proposal_modes.linear_bandit as lb
from importlib import reload
reload(lb)
...
LINEAR_BANDIT_CONFIG = {
    ...
    "model_kwargs": {
        ...
        "lambda_1": 1e-6,
        "lambda_2": 1e-6,
        "threshold_lambda": 100.0,  # more aggressive pruning than default
        "fit_intercept": True,
    },
    "acquisition": {"beta": 0.5, "eta": 1},
    "optimization": {
        "ensamble_size": 5000,  # small for quick test
        "patchwork_params": {
            "apply_rotation": True,
            "rotation_prob": 0.2,
            "rotation_type": "Mixed",
...
proposals_lb = lb.propose_linear_bandit_constraints(
    occ_matr_list=occ_matrices[:],
    N=10,
    natoms=2,
    debug=True,
...
print(f"\nGenerated {len(proposals_lb)} linear bandit proposals")
for i, p in enumerate(proposals_lb):
    print(f"  Proposal {i}: {len(p)} atoms")
